...
if __name__ == '__main__':
    ...
    print('The comp videos will be saved in: {0}'.format(os.path.join(args.example_path, 'composite')))
    ...
    for vdx, example in enumerate(examples):
        ...
        # check frame shape
        if fg_frames[0].shape[:2] != bg_frames[0].shape[:2]:
            for i, frame in enumerate(fg_frames):
                resized_frame = cv2.resize(frame, (bg_frames[i].shape[1], bg_frames[i].shape[0]))
                fg_frames[i] = resized_frame
