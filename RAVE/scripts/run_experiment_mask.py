...
from pipelines.sd_controlnet_rave_mask import RAVE
...
def init_device():
    if torch.cuda.is_available():
        device = torch.device('cuda:0')  # Use GPU 1
    else:
...
def init_paths(input_ns):
    ...
    # Mask if applied
    if hasattr(input_ns, 'mask_video_name') and input_ns.mask_video_name:
        input_ns.mask_video_path = f'{const.MP4_PATH}/{input_ns.mask_video_name}.mp4'
    else:
        input_ns.mask_video_path = None  
...
def run(input_ns):
    ...
    # Process mask 
    if hasattr(input_ns, 'mask_video_name') and input_ns.mask_video_name:
        input_ns.mask_pil_list = vgu.prepare_video_to_grid(input_ns.mask_video_path, input_ns.sample_size, input_ns.grid_size, input_ns.pad)
        print(f'Mask frame count: {len(input_ns.mask_pil_list)}')
    else:
        input_ns.mask_pil_list = None
        print("No mask video provided.")
    ...
    start_time = datetime.datetime.now()
    if '-' in str(input_ns.controlnet_conditioning_scale):
        res_vid, control_vid_1, control_vid_2 = CN(input_dict, mask_pil_list=input_ns.mask_pil_list)
    else:
        res_vid, control_vid = CN(input_dict, mask_pil_list=input_ns.mask_pil_list)
    end_time = datetime.datetime.now()
