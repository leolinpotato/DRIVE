...
class RAVE(nn.Module):
    ...
    @torch.no_grad()
    def prepare_binary_mask(self, mask_pil, width, height):
        # Resize and normalize mask
        mask_image = mask_pil.resize((width, height)).convert("L")
        mask_array = np.array(mask_image) / 255.0  # Normalize to [0, 1]
        #fix
        mask_array = (mask_array > 0.5).astype(np.float32)
        mask_tensor = torch.tensor(mask_array, dtype=torch.float32).unsqueeze(0).unsqueeze(0)  # Shape: [1, 1, height, width]
        mask_tensor = mask_tensor.to(self.device, self.dtype)
        # Debugging: Check mask values
        # print(f"Mask Shape: {mask_tensor.shape}")
        # print(f"Mask Min Value: {mask_tensor.min().item()}, Max Value: {mask_tensor.max().item()}")
    ...
    @torch.no_grad()
    def denoising_step(self, latents, control_image, text_embeddings, t, guidance_scale, current_sampling_percent, mask=None):
        ...
        # Debugging shapes(latent small)
        # print(f"Latents shape: {latents.shape}")
        # print(f"Mask shape: {mask.shape if mask is not None else 'None'}")

        # Apply mask to blend noise predictions
        if mask is not None:
            # Ensure mask matches latent spatial dimensions
            mask = torch.nn.functional.interpolate(mask, size=(latents.shape[2], latents.shape[3]), mode="nearest")
            mask = mask.unsqueeze(1) if mask.ndim == 3 else mask  # Add channel dimension if missing
            #print(f"Resized Mask shape: {mask.shape}")  # Debug resized mask

            # Apply mask to the noise prediction
            noise_pred = noise_pred * mask + noise_pred_uncond * (1 - mask)
    ...
    @torch.no_grad()
    def shuffle_latents(self, latents, control_image, indices, mask = None):
        rand_i = torch.randperm(self.total_frame_number).tolist()
        
        latents_l, controls_l, masks_l, randx = [], [], [], []
        ...
        return latents, indices, control_image, mask
    
    @torch.no_grad()
    def batch_denoise(self, latents, control_image, indices, t, guidance_scale, current_sampling_percent, mask = None):
        ...
        for idx in range(len(control_split)):
            ...
            if mask is not None:
                # Apply mask to the corresponding batch
                batch_mask = mask[idx * self.batch_size:(idx + 1) * self.batch_size]
            else:
                batch_mask = None

            latents = self.denoising_step(latents_split[idx], control_split[idx], txt_embed, t, guidance_scale, current_sampling_percent, mask = batch_mask)
    ...
    @torch.no_grad()
    def reverse_diffusion(self, latents=None, control_image=None, guidance_scale=7.5, indices=None, mask = None):
        ...
        with torch.autocast('cuda'):
            ...
            for i, t in tqdm(enumerate(self.scheduler.timesteps), desc='reverse_diffusion'):
                ...
                if self.is_shuffle:
                    latents, indices, control_image, mask = self.shuffle_latents(latents, control_image, indices, mask)
                    
                if self.cond_step_start < current_sampling_percent:
                    latents, indices, controls = self.batch_denoise(latents, control_image, indices, t, guidance_scale, current_sampling_percent, mask)
                else:
                    latents, indices, controls = self.batch_denoise(latents, control_image, indices, t, 0.0, current_sampling_percent, mask)
...
#//////
    ...
    @torch.no_grad()
    def __call__(self, input_dict, mask_pil_list=None):
        ...
        if mask_pil_list:
            ...
            prepared_masks = [self.prepare_binary_mask(mask_pil, img_batch.size(-1), img_batch.size(-2)) for mask_pil in mask_pil_list]

            # Ensure masks align with the batch size
            if len(prepared_masks) == img_batch.size(0):
                # Stack masks into a batch
                mask_batch = torch.cat(prepared_masks, dim=0)  # Shape: [batch_size, 1, height, width]
            elif len(prepared_masks) == 1:
                # Replicate single mask across the batch size
                mask_batch = prepared_masks[0].expand(img_batch.size(0), -1, -1, -1)  # Shape: [batch_size, 1, height, width]
            else:
                raise ValueError("Number of masks does not match the batch size.")
        ...
        latents_denoised, indices, controls = self.reverse_diffusion(latents_inverted, control_batch, self.guidance_scale, indices=indices, mask = mask_batch)#watch
